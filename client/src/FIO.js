...
import { demographieText, fioText, planText } from "./Text";
...
export default ({lg, setFenetre, fenetre, dataPatient})=>{
  ...
  let mediane = calculMediane(plage) 
  const graph = {
      labels: data.horaires,
      datasets: [{
        label: fioText.label1[lg],
        data: data.valeurs,
  ...
  const download = (event) => {
    ...
    imageLink.download = 'chart'+ dataPatient.fio2[0].nom +'-'+ dataPatient.infos[4] +'.jpg';
    imageLink.href = canvas.toDataURL('image/jpg',1)
    imageLink.click();
  };

const pourcentageBAS = 21
const pourcentageHAUT = 28
      ...
      <Grid container spacing={2} >
          ...
          <Grid item xs={10} sm={10} md={10} container spacing={1}>
            ...
            {data.valeurs.length == 0 ?
                <Grid item xs={12} sm={12} md={12}>
                  <Typography variant='h4' style={styles.titre}>{fioText.titre[lg]}</Typography>
                  <Item>
                  {!doneFetching ?
                    <div>
                  ...
                  <Grid item xs={12} sm={12} md={12} container spacing={1} justifyContent='center' alignItems="center">
                    ...
                    <Grid container item xs={3} sm={3} md={3} justifyContent='center' alignItems="center">
                      <Grid style={{textAlign:"center"}} item xs={12} sm={12} md={12} >
                        <Item style={{color:isDark? '#0091ea':'#2c387e'}}>
                          <Typography variant="h5" style={{marginBottom:'1px'}}>{fioText.mediane[lg]}</Typography>
                          <Typography variant="h5" style={{marginTop:'1px', color:isDark? '#00b0ff': "#03a9f4"}}>{mediane}</Typography>
                          <Item style={{marginTop:'10px'}}> 
                            <Typography variant="h5" style={{color: isDark? '#0091ea':'#2c387e', marginBottom:'1px'}}>{fioText.IQR[lg]}</Typography>
                            <Typography variant="h5" style={{color: isDark? '#00b0ff': "#03a9f4", marginTop:'1px'}}>{pourcentageBAS}% - {pourcentageHAUT}%</Typography>
                          </Item>
                        </Item>
                      </Grid>
                    </Grid>
